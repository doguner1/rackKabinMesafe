...
class RackNameEditModal extends BaseModal {
  /**
   * @param {string} modalId - Modal element ID'si (opsiyonel)
   */
  constructor(modalId = 'rack-name-edit-modal') {
    // BaseModal'dan farklı olarak, bu modal DOM'da hazır değil, oluşturulması gerekiyor
    // Bu yüzden önce modalı oluşturup sonra super() çağrısı yapacağız
    RackNameEditModal.createModalInDOM(modalId);
    super(modalId);
    this.init();
  }
   ...
   */
  init() {
    // Butonlar zaten createModalInDOM içinde ayarlandı
  }

  /**
   * Rack isim düzenleme modalını DOM'da oluşturur
   * @param {string} modalId - Oluşturulacak modalın ID'si
   */
  static createModalInDOM(modalId) {
